...
const get_airports_in_radius = async (origin_cords, radius) => {
  ...
  for (const airport of airports) {
    if (
      origin_cords.latitude - parseFloat(airport[5]) == 0 &&
      origin_cords.longitude - parseFloat(airport[6]) == 0
    ) {
      continue;
    }
    ...
    const distance = calculateDistance(
      origin_cords.latitude,
      origin_cords.longitude,
      latitude,
      longitude
    );
...
const close_airports = async (
  ...
  const airports_in_radius = await get_airports_in_radius(
    origin_cords,
    max_radius
  );
  ...
  for (const airport_in_radius of airports_in_radius) {
    ...
    if (
      weatherData.current.temperature2m > maxTemp ||
      weatherData.current.temperature2m < minTemp
    )
      continue;
